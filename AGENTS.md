...
IAM engine for permissions, policies, roles, access control, and authorization logic.

## Allowed Work

- Bug fixes and small enhancements for IAM UI, permissions, policies, roles, and access-control workflows.
- Tests, documentation, and frontend consistency improvements that follow existing Ember patterns.
- Non-breaking updates to permission-facing UI when behavior is clear and validated.

## Restricted Work

- Authorization semantics, permission defaults, or role policy changes without explicit review.
- Security-sensitive behavior changes without human approval.
- Documentation-sensitive IAM behavior changes without checking `fleetbase/fleetbase.io` and any relevant API specs.
